...
if __name__ == '__main__':
    ...
    cm = confusion_matrix(true_labels_class, predictions_class)
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt='d', cmap='OrRd', annot_kws={"size": 14, "weight": "bold"})
    plt.xlabel('Predicted Label', fontsize=14)
    plt.ylabel('True Label', fontsize=14)
    plt.title("ResNet", fontsize=16)
    plt.savefig('ResNet_Multi-class Classification.svg', format='svg', dpi=300)
    plt.show()
